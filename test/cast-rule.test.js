 ...
 *   - expressions continued on the next line stay silent (review blocker on PR #57),
 *     with CRLF line endings too;
 *   - comma-separated segments are judged one by one;
...
const SILENT = [
  ...
  ['continued with : at column 0 of the wrap', 'int n = input.bool(true) ? 1\n  : 0\nplot(n)'],
  ['continued with a method call', 'string s = input.float(1.5)\n  .tostring()\nplot(close)'],
  ['continued with and', 'bool b = input.string("x")\n  == "x" and close > open\nplot(b ? 1 : 0)'],
  ['continued with and at the line start', 'bool b = input.int(1)\n  and true\nplot(close)'],
  ['continued with or at the line start', 'bool b = input.int(1)\n  or true\nplot(close)'],
  ['continued with arithmetic', 'int n = input.float(1.5)\n  * 2\nplot(n)'],
  ['continued with [', 'int n = input.float(1.5)\n  [1]\nplot(n)'],
  ['continued with a comma', 'int n = input.float(1.5)\n  , m = 2\nplot(n)'],
  ['continued after a blank and a comment line', 'int n = input.float(1.5)\n\n// why\n  * 2\nplot(n)'],
  ['next line indented deeper (any token)', 'int n = input.float(1.5)\n    + 2\nplot(n)'],
];
