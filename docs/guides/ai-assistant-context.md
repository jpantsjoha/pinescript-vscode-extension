# Pine Script v6 Expert System - AI Assistant Context

Long-form Pine Script v6 language reference for AI coding agents (originally written for
Gemini, equally usable by any assistant). For the shorter directives-style companion, see
[AI-ASSISTANT-GUIDE.md](./AI-ASSISTANT-GUIDE.md); for the project's own architecture and
rules, see `CLAUDE.md` at the repo root.
...
| `docs/guides/AI-ASSISTANT-GUIDE.md` | **AI agent workflows** and Pine Script best practices | Understand code generation patterns |
...
The repo's own `mcp/pinescript-mcp-server.js` validates Pine Script through the same
`AccurateValidator` + `documentChecks` pair the extension runs — see
[mcp-integration.md](./mcp-integration.md) for setup, tools and troubleshooting.

Two third-party MCP servers (`mcp/pinescript-mcp-server/`, `mcp/tradingview-mcp/`) can be
vendored locally for market-data and syntax-checking workflows; both are gitignored, so
they exist only on machines that installed them and are not part of a clean checkout.
...
Deprecated-constant detection (`plot.style_dashed` → warn, use `plot.style_linebr`) exists only
in the dead `ComprehensiveValidator` (see `CLAUDE.md`), not in the shipping `AccurateValidator`.
The live validator will not warn on v5 constants — check the table above yourself.
...
**Usage:**
```bash
# Validate all .pine files under examples/
npm run qa:pinescript

# Validate a specific file
node validate-cli.js path/to/your/script.pine
```
...
- **AI Workflows:** `docs/guides/AI-ASSISTANT-GUIDE.md`
