...
function referenceNames(html) {
  ...
  const byName = new Map(); // name -> description ('' when the page carries none)
  $('.tv-pine-reference-item').each((_, item) => {
    const id = $(item).attr('id') || '';
    const m = id.match(/^(const|var)_(.+)$/);
    if (!m || !m[2].includes('.')) return;
    if (byName.has(m[2])) return; // duplicated legacy ids: first occurrence wins
    const description = $(item).find('.tv-pine-reference-item__text').first().text().trim().replace(/\s+/g, ' ');
    byName.set(m[2], description);
  });
  const names = [...byName.keys()].sort();
  const descriptions = {};
  for (const name of names) {
    if (byName.get(name)) descriptions[name] = byName.get(name);
  }
  return { names, descriptions };
}

function emitNames({ names, descriptions }) {
  return [
    '/**',
    ...
    `export const REFERENCE_NAME_DESCRIPTIONS: Record<string, string> = ${JSON.stringify(descriptions, null, 2)};`,
    '',
  ].join('\n');
}
...
async function main() {
  ...
  if (args.includes('--names-only')) {
    ...
    console.log(`wrote ${NAME_OUTPUTS.map(o => path.relative(ROOT, o)).join(', ')} (${names.names.length} names)`);
    return;
  }
  ...
  const namesTs = emitNames(names);
  for (const out of NAME_OUTPUTS) fs.writeFileSync(out, namesTs);
  console.log(`wrote ${[...OUTPUTS, ...NAME_OUTPUTS].map(o => path.relative(ROOT, o)).join(', ')} (${names.names.length} constant/variable names)`);
}
